...
from engine.config import (
    ...
    MEMORY_CONFIG,
    THEME,
    set_theme,          # re-export for convenience
    THEME_PRESETS,      # re-export for convenience
)
from engine.i18n_labels import get_label, get_label_formatted

# ── Pure i18n layer (no Streamlit dependency) ───────────────────────
# Call set_language() before generating plots to set the active language.
_current_lang = "en"
...
def create_volcano_plot(
    ...
    # ── Themed figure ────────────────────────────────────────────────
    fig, ax = _themed_fig(cfg["figsize"])
    ...
    for cat in CATEGORY_ORDER:
        ...
        ax.scatter(
            ...
            linewidths=THEME["point_edge_width"],
            zorder=style["zorder"],
        )
    ...
    ax.text(
        ...
        fontsize=7.5, color=THEME["text_subtle"], va="bottom",
    )
    ax.text(
    ...
    ax.set_xlabel(xlabel, fontsize=cfg["font_axes"], color=THEME["text_muted"])
    ax.set_ylabel(cfg["ylabel"], fontsize=cfg["font_axes"], color=THEME["text_muted"])
        ...
        if len(texts) > 1:
            try:
                adjust_text(
                    texts,
                    ax=ax,
                    arrowprops=dict(arrowstyle="-", color=THEME["amber"], lw=0.5, alpha=0.6),
...
def create_pca_plot(
    ...
    fig, ax = _themed_fig((width, height))
    ...
    ax.set_xlabel(
        ...
        fontsize=cfg["font_axes"], color=THEME["text_muted"],
    )
    ax.set_ylabel(
    ...
    ax.set_title(pca_title, fontsize=cfg["font_title"], fontweight="bold",
...
def create_ma_plot(
    ...
    """Generate an MA plot (mean expression vs. log2 fold-change).

    The MA plot visualises the relationship between average expression
    (baseMean) and effect size (log2FC), highlighting significantly
    up- and down-regulated genes.

    Parameters
    ----------
    ma_df : pd.DataFrame
        DataFrame prepared by :func:`prepare_ma_data`, with columns
        ``log10_baseMean``, ``log2FoldChange``, and ``category``.
    test_level, reference_level : str
        Condition labels for the Y-axis annotation.
    legend_loc : str
    ...
    ax.set_xlabel(cfg["xlabel"], fontsize=cfg["font_axes"], color=THEME["text_muted"])
    ax.set_ylabel(
        f'{cfg["ylabel"]} ({test_level} vs {reference_level})',
        fontsize=cfg["font_axes"],
        color=THEME["text_muted"],
    )
    ax.set_title(cfg["title"], fontsize=cfg["font_title"], fontweight="bold",
                 color=THEME["text"], pad=15)
    ...
    ax.annotate(
        ...
        fontstyle="italic",
        color=THEME["text_muted"],
        bbox=dict(boxstyle="round,pad=0.4", facecolor=THEME["annot_bg"],
                  edgecolor=THEME["annot_edge"], alpha=THEME["annot_alpha"]),
    )
...
def create_heatmap(
    ...
    """Generate a clustered heatmap of top differentially expressed genes.

    Uses ``seaborn.clustermap`` to perform hierarchical clustering on
    both genes (rows) and samples (columns).  A colour bar at the top
    indicates each sample's condition.

    Parameters
    ----------
    heatmap_df : pd.DataFrame
        Z-score normalised matrix (genes x samples), produced by
        :func:`prepare_heatmap_data`.  Must have
        ``attrs["conditions"]`` attached.
    ...
    g = sns.clustermap(
        ...
        tree_kws={"linewidths": 1.0, "colors": THEME["text_muted"]},
    ...
    # ── Apply dark theme to the clustermap figure ────────────────────
    g.fig.patch.set_facecolor(THEME["bg"])
    for _ax in [g.ax_heatmap, g.ax_row_dendrogram, g.ax_col_dendrogram]:
        _ax.set_facecolor(THEME["bg"])
    if g.ax_col_colors is not None:
        g.ax_col_colors.set_facecolor(THEME["bg"])
    ...
    # ── Colorbar ─────────────────────────────────────────────────────
    g.cax.set_facecolor(THEME["bg"])
    g.cax.set_ylabel(t("plot.heatmap_zscore_label"), fontsize=10, rotation=90,
                     labelpad=8, color=THEME["text_muted"])
    g.cax.yaxis.set_label_position("left")
    g.cax.tick_params(labelsize=8, colors=THEME["text_muted"])

    # ── Condition legend ────────────────────────────────────────────
    if lut:
        from matplotlib.patches import Patch
        legend_handles = [Patch(facecolor=c, edgecolor=THEME["border"], label=l)
