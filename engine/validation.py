"""
validation.py — Input data validation, normalization, and quality reporting.
...
ValidationFinding
    Dataclass — a single finding from a validation check.

ValidationReport
    Dataclass — accumulates findings, exposes ``n_errors``, ``n_warnings``,
    ``n_info``, ``passed`` properties and ``to_dict()`` for JSON export.
...
check_sample_overlap(counts_df, metadata_df)
    → Returns overlap statistics between counts and metadata sample names.

align_samples(counts_df, metadata_df)
    → Verifies that samples match and aligns them.

get_condition_levels(metadata_df, condition_col)
    → Returns unique condition levels from the metadata.
...
filter_low_expression_genes(counts_df, …)
    → Removes lowly-expressed genes before DESeq2.

downcast_counts(counts_df)
    → Reduces memory by casting counts to int32.

Pre-analysis advisory checks
------------------------------
check_class_imbalance(metadata_df, condition_col)
    → Detects severe class imbalance between condition groups.

check_sparse_data(counts_df)
    → Returns True if every gene has at least one zero across samples.

Strict validation checks
--------------------------
check_duplicate_samples(metadata_df)
    → Returns list of duplicated sample names in the metadata index.

check_duplicate_gene_ids(counts_df)
    → Returns list of duplicated gene IDs in the counts index.

check_condition_nan(metadata_df, condition_col)
    → Counts NaN or empty-string values in the condition column.

check_metadata_types(metadata_df, condition_col)
    → Checks whether the condition column appears numeric vs. categorical.

check_alignment_quality(overlap, report)
    → Analyses sample alignment quality and adds findings to report.

check_post_filter_sanity(n_before, n_after, report)
    → Warns when gene filtering is too aggressive.

check_filter_params(min_samples_expressing, n_samples, report)
    → Warns when min_samples_expressing exceeds 90% of total samples.

Usage example
--------------
    from engine.validation import normalize_indices, align_samples
...
from engine.config import DESEQ2_DEFAULTS, GENE_FILTER_DEFAULTS, MEMORY_CONFIG
...
class ValidationReport:
    """Accumulates findings from all validation steps.

    Pure Python dataclass — no Streamlit dependency, fully picklable.
    Each finding carries an ``i18n_key`` so the UI can translate messages.
    """
    ...
    def to_dict(self) -> dict:
        """JSON-serialisable representation for export."""
        return {
            "passed": self.passed,
            ...
            "findings": [
                {
                    ...
                    "i18n_key": f.i18n_key,
                    "i18n_params": f.i18n_params,
                    "details": (
                        f.details if isinstance(f.details, (str, int, float, bool, list, dict, type(None)))
                        else str(f.details)
                    ),
                }
...
# ──────────────────────────────────────────────────────────────────────
# Strict validation — structural checks
# ──────────────────────────────────────────────────────────────────────

def check_duplicate_samples(metadata_df: pd.DataFrame) -> list[str]:
    """Detect duplicate sample names in the metadata index.

    A sample is considered duplicated if its index value appears more than
    once.  DESeq2 requires unique sample identifiers; duplicates would
    silently corrupt the design matrix.

    Parameters
    ----------
    metadata_df : pd.DataFrame
        Metadata with sample names as the index.

    Returns
    -------
    list[str]
        Sorted list of duplicated sample names (empty if none).
    """
    dupes = metadata_df.index[metadata_df.index.duplicated(keep=False)]
    return sorted(set(dupes.tolist()))


def check_duplicate_gene_ids(counts_df: pd.DataFrame) -> list[str]:
    """Detect duplicate gene IDs in the counts index.

    Duplicate gene IDs cause row ambiguity and can silently produce
    incorrect fold-change estimates.

    Parameters
    ----------
    counts_df : pd.DataFrame
        Gene-by-sample count matrix with gene IDs as the index.

    Returns
    -------
    list[str]
        Sorted list of duplicated gene IDs (empty if none).
    """
    dupes = counts_df.index[counts_df.index.duplicated(keep=False)]
    return sorted(set(dupes.tolist()))


def check_condition_nan(
    metadata_df: pd.DataFrame,
    condition_col: str = "condition",
) -> int:
    """Count NaN or empty-string values in the condition column.

    Both ``NaN`` and whitespace-only strings are treated as missing.
    Samples with missing conditions cannot be assigned to a group and
    will cause DESeq2 to fail.

    Parameters
    ----------
    metadata_df : pd.DataFrame
        Metadata DataFrame (index = sample names).
    condition_col : str
        Name of the column containing condition/group labels.

    Returns
    -------
    int
        Number of samples with missing or empty condition values (0 if clean).
    """
    if condition_col not in metadata_df.columns:
        return 0
    col = metadata_df[condition_col]
    n_null = int(col.isna().sum())
    # Count empty/whitespace strings only among non-null values
    # (NaN.astype(str) → "nan", never "", so no double-counting risk)
    non_null = col.dropna().astype(str).str.strip()
    n_empty = int((non_null == "").sum())
    return n_null + n_empty


def check_metadata_types(
    metadata_df: pd.DataFrame,
    condition_col: str = "condition",
) -> dict:
    """Check if the condition column appears to be numeric rather than categorical.

    DESeq2 expects categorical condition labels (e.g. ``"control"`` /
    ``"treated"``).  Purely numeric values (``0``, ``1``) are valid but
    may indicate the user forgot to map codes to labels, which makes
    ...
    -------
    dict
        ``{"is_numeric": bool, "sample_values": list[str]}`` where
        *sample_values* contains up to 5 representative unique values.
    """
    if condition_col not in metadata_df.columns:
        return {"is_numeric": False, "sample_values": ""}

    col = metadata_df[condition_col]
    unique_vals = col.dropna().unique()

    # Check if all unique values can be parsed as numbers
    try:
        numeric = [float(v) for v in unique_vals]
        is_numeric = True
    except (ValueError, TypeError):
        is_numeric = False

    sample_str = ", ".join(str(v) for v in unique_vals[:5])
    return {"is_numeric": is_numeric, "sample_values": sample_str}


# ──────────────────────────────────────────────────────────────────────
# Strict validation — alignment quality
# ──────────────────────────────────────────────────────────────────────
...
def check_alignment_quality(
    ...
    """Analyse sample alignment quality and add findings to *report*.

    Parameters
    ----------
    overlap : dict
        Output of :func:`check_sample_overlap`.
    report : ValidationReport
        Report to populate with findings.
    """
    n_common = overlap.get("n_common", 0)
    n_counts = overlap.get("n_counts", 0)
    n_metadata = overlap.get("n_metadata", 0)
    only_counts = overlap.get("only_in_counts", set())
    only_meta = overlap.get("only_in_metadata", set())

    # High sample loss from counts
    if n_counts > 0 and only_counts:
        pct_lost = len(only_counts) / n_counts * 100
        if pct_lost > 50:
            report.add(
                "alignment_high_loss_counts", Severity.WARNING,
                "strict.alignment_high_loss",
                {"pct_lost": pct_lost, "n_dropped": len(only_counts),
                 "n_total": n_counts, "source": "counts"},
                details=sorted(only_counts)[:20],
            )

    # High sample loss from metadata
    if n_metadata > 0 and only_meta:
        pct_lost = len(only_meta) / n_metadata * 100
        if pct_lost > 50:
            report.add(
                "alignment_high_loss_metadata", Severity.WARNING,
                "strict.alignment_high_loss",
                {"pct_lost": pct_lost, "n_dropped": len(only_meta),
                 "n_total": n_metadata, "source": "metadata"},
                details=sorted(only_meta)[:20],
            )

    # Case-insensitive near-duplicates
    if only_counts and only_meta:
        lower_counts = {s.lower(): s for s in only_counts}
        lower_meta = {s.lower(): s for s in only_meta}
        collisions = set(lower_counts) & set(lower_meta)
        if collisions:
            pairs = [
                f"{lower_counts[c]} ↔ {lower_meta[c]}"
                for c in sorted(collisions)[:5]
            ]
            report.add(
                "alignment_case_mismatch", Severity.WARNING,
                "strict.alignment_case_mismatch",
                {"pairs": ", ".join(pairs)},
                details=[
                    {"counts": lower_counts[c], "metadata": lower_meta[c]}
                    for c in sorted(collisions)
                ],
            )

    # Info: samples dropped
    if only_counts or only_meta:
        dropped = sorted(only_counts | only_meta)[:20]
        report.add(
            "alignment_dropped", Severity.INFO,
            "strict.alignment_dropped",
            {"dropped_list": ", ".join(dropped)},
            details={"only_in_counts": sorted(only_counts),
                     "only_in_metadata": sorted(only_meta)},
        )


# ──────────────────────────────────────────────────────────────────────
# Strict validation — post-filter sanity
# ──────────────────────────────────────────────────────────────────────

def check_post_filter_sanity(
    n_before: int,
    n_after: int,
    report: ValidationReport,
) -> None:
    """Check that gene filtering didn't remove too many genes.
    ...
    if n_after < 10:
        report.add(
            "post_filter_critical", Severity.ERROR,
            "strict.post_filter_critical",
            {"n_after": n_after, "n_before": n_before},
        )
    elif n_after < 100:
        report.add(
            "post_filter_low", Severity.WARNING,
...
def check_filter_params(
    ...
    """Warn if filter parameters are unreasonably aggressive.

    Parameters
    ----------
    min_samples_expressing : int
        Resolved (non-zero) threshold for sample expression.
    n_samples : int
        Total number of samples.
    report : ValidationReport
        Report to populate.
    """
    if n_samples > 0 and min_samples_expressing > n_samples * 0.9:
        pct = round(min_samples_expressing / n_samples * 100)
        report.add(
            "filter_param_aggressive", Severity.WARNING,
            "strict.filter_param_aggressive",
            {"min_samples": min_samples_expressing,
             "pct": pct, "n_samples": n_samples},
        )
