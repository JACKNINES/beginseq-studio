...
from engine.config import SCRNA_CONFIG
...
def load_h5ad(file_bytes: bytes) -> ad.AnnData:
    """
    Load an AnnData object from raw .h5ad bytes.
...
def _try_extract_gene_id_map(metadata: Optional[FileData]) -> Optional[dict]:
    """
...
def integrate_10x_files(
    ...
    features: FileData,
    barcodes: FileData,
    metadata: Optional[FileData] = None,
    ...
    try:
        # ── Write matrix.mtx.gz ─────────────────────────────────────
        mtx_bytes = matrix.content
        mtx_name = matrix.name
        ...
        bc_bytes = barcodes.content
        bc_name = barcodes.name
        ...
        gene_id_map = None
        if _n_feat_cols == 1 and metadata is not None:
            gene_id_map = _try_extract_gene_id_map(metadata)
        ...
        if metadata is not None:
            meta_result = detect_and_merge_metadata(
                adata, metadata, force_target=metadata_force_target,
            )
...
def _read_metadata_file(file: FileData) -> pd.DataFrame:
...
def detect_and_merge_metadata(
    ...
    metadata : FileData
        File content + name for a CSV/TSV file.
    ...
    """
    meta_df = _read_metadata_file(metadata)
...
def run_scrna_pipeline(
    ...
    _total_elapsed = _time.monotonic() - _pipeline_t0
    from engine.audit import build_scrna_audit
    adata.uns["audit_log"] = build_scrna_audit(
        adata, params, elapsed_seconds=_total_elapsed,
    )
