"""
engine/data_io.py — Reading and writing data files.
...
read_counts_file(file_obj)
    → Reads the counts matrix from a BinaryIO-compatible file object.

read_metadata_file(file_obj)
    → Reads the metadata file from a BinaryIO-compatible file object.
...
from engine.config import FILE_CONFIG, MEMORY_CONFIG
...
def extract_from_zip(uploaded_file: BinaryIO) -> tuple[io.BytesIO, str]:
    """
    ...
    uploaded_file : BinaryIO
        File-like object with .seek(), .read(), and .name attributes.
...
def read_counts_file(uploaded_file: BinaryIO) -> pd.DataFrame:
    """
    Reads a raw counts matrix from a file-like object.
    ...
    try:
        ...
        raise ValueError(
            f"Error reading counts matrix '{real_name}': "
            f"{str(e)}. Please check the file format."
        ) from e

    return df


def read_metadata_file(uploaded_file: BinaryIO) -> pd.DataFrame:
    """
    Reads a metadata file from a file-like object.
    ...
    try:
        ...
        raise ValueError(
            f"Error reading metadata '{real_name}': "
            f"{str(e)}. Please check the file format."
        ) from e
